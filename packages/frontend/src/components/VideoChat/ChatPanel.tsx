...
export default function ChatPanel({ messages, onSendMessage, onClose, isFullScreen = false }: ChatPanelProps) {
  ...
  return (
    <div className={isFullScreen
      ...
      : "absolute right-2 sm:right-4 bottom-20 sm:bottom-24 w-[calc(100vw-1rem)] sm:w-80 max-w-80 bg-white/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border border-gray-200"
    }>
      {/* Modern Header */}
      ...
      {/* Messages Area */}
      <div ref={messagesParent} className={`${isFullScreen ? 'flex-1' : 'h-56 sm:h-72'} overflow-y-auto p-3 sm:p-4 space-y-2 sm:space-y-3 bg-gray-50/50`}>
        {messages.length === 0 ? (
