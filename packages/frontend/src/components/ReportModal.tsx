...
export default function ReportModal({ onSubmit, onClose, isLoading = false }: ReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-3 sm:p-4 animate-fadeIn">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-sm sm:max-w-md w-full overflow-hidden shadow-2xl animate-scaleIn max-h-[90vh] overflow-y-auto">
        {/* Header */}
          ...
          {step === 'reason' ? (
            ...
            <>
              ...
              <textarea
                ...
                className="w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border-2 border-gray-200 focus:border-orange-500 focus:ring-0 outline-none text-sm resize-none h-20 sm:h-24"
                maxLength={500}
              />
              ...
              <div className="mt-3 sm:mt-4 space-y-2 sm:space-y-3">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-3 sm:py-3.5 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-bold text-sm sm:text-base rounded-xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-[1.02] active:scale-[0.98]"
                >
                  {isLoading ? (
                ...
                <button
                  onClick={onClose}
                  className="w-full py-2.5 sm:py-3 text-gray-600 hover:text-gray-800 font-medium text-sm sm:text-base transition-colors"
                >
                  Cancel
                </button>
              </div>
